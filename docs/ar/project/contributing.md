...
> ```
> 
> بالمناسبة، قبل تنفيذ الالتزام، إذا كنت تقوم بتعديل أي ملف `.sh` أو سكريبت bash، تأكد من فحص مجلد `scripts/` وتشغيل `./check.sh` للتحقق من المسافات الزائدة أو الفارغة. إذا فشل، شغّل `./cleanup.sh`. تأكد أيضًا من تنسيق سكريبتات الشل باستخدام `shfmt`.
