...
> ```
> 
> Кстати, перед коммитом, если вы меняете любой файл `.sh` или bash-скрипт, обязательно проверьте каталог `scripts/` и запустите `./check.sh` для поиска лишних пробелов или пустых строк. Если скрипт завершится с ошибкой, запустите `./cleanup.sh`. Также убедитесь, что shell-скрипты отформатированы с помощью `shfmt`.
